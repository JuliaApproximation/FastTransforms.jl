...

{
    ...
    "title": "Introduction",
    "category": "section",
    "text": "In numerical analysis, it is customary to expand a function in a basis:f(x) = sum_ell=0^infty f_ell phi_ell(x)It may be necessary to transform our representation to one in a new basis, say, psi_m(x)_mge0:f(x) = sum_m=0^infty g_m psi_m(x)In many cases of interest, both representations have finite dimension n and we seek a fast method (faster than mathcalO(n^2)) to transform the original coefficients f_ell to the new coefficients g_m.A similar problem arises when we wish to evaluate f at a set of points x_m_m=0^n. We wish to transform coefficients of f to values at the set of points in fewer than mathcalO(n^2) operations."
},
